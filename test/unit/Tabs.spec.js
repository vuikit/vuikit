...
describe('VkTabs', () => {
  ...
  it('should change tab content on href click', done => {
    expect($vm.$el.querySelector('.uk-list li').innerHTML.trim()).toBe('Content Tab 1')
    triggerEvent($vm.$el.querySelectorAll('.uk-tab li a')[1], 'click')
    waitForUpdate(() => {
      expect($vm.$el.querySelector('.uk-list li').innerHTML.trim()).toBe('Content Tab 2')
    }).then(done)
  })
